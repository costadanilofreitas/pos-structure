...
from bustoken import TK_FISCALWRAPPER_PROCESS_REQUEST, TK_FISCALWRAPPER_SEARCH_REQUEST, TK_FISCALWRAPPER_REPRINT, \
    ...
    TK_SAT_PAYMENT_STATUS, TK_FISCALWRAPPER_CANCEL_ORDER, TK_FISCALWRAPPER_RE_SIGN_XML, \
...
from old_helper import OrderTaker, round_half_away_from_zero, remove_xml_namespace
from ibpttaxprocessor import IbptTaxProcessor
from messagehandler import EventHandler
from msgbus import TK_SYS_NAK, TK_SYS_ACK, MBEasyContext, MBException, FM_PARAM, MBMessage
...
class FiscalWrapperEventHandler(EventHandler):
    def __init__(self, mbcontext, fiscal_processor, fiscal_printer, nfce_connectivity_tester, nfce_situation_checker, order_disabler, nfce_canceler, fiscal_parameter_controller, xml_enviados, xml_erros, xml_path, fiscal_sent_dir, nf_type, versao_ws, ibpt_tax_processor, order_retriever_service, xml_inconsistentes, nfce_contingencia, period_to_retry_orders_with_exception):
        # type: (MBEasyContext, FiscalProcessor, printer.FiscalPrinter, NfceConnectivityTester, NfceSituationChecker, OrderDisabler, NfceCanceler, FiscalParameterController, str, str, str, str, str, int, IbptTaxProcessor, OrderRetrieverService, str, NfceContingencia, str) -> None
        super(FiscalWrapperEventHandler, self).__init__(mbcontext)
        self.fiscal_processor = fiscal_processor
        self.fiscal_printer = fiscal_printer
    ...
    def get_handled_tokens(self):
        return [TK_FISCALWRAPPER_PROCESS_REQUEST, TK_FISCALWRAPPER_SEARCH_REQUEST, TK_FISCALWRAPPER_REPRINT,
                ...
                TK_FISCALWRAPPER_GET_LAST_ORDER_ID, TK_FISCALWRAPPER_GET_ORDERS_DATA, TK_FISCALWRAPPER_GET_CERTIFICATE_EXPIRATION_DAYS,
                TK_FISCALWRAPPER_GET_CONTINGENCY_STATUS, TK_FISCALWRAPPER_CHANGE_CONTINGENCY_STATUS]
    ...
    def handle_message(self, msg):
        ...
        try:
            if msg.token == TK_FISCALWRAPPER_PROCESS_REQUEST:
                ...
                response = self._handle_sat_payment_status_request()
    ...
    def _handle_get_nftype(self, msg):
                ...
                for corrected_line in dict_list:
                    ...
                    if corrected_line_dict['line_number'] == line_number and \
                       ...
                       corrected_line_dict['level'] == level:
                        ...
                        line.set('defaultQty', str(int(corrected_line_dict['default_qty'])))

        else:
            data = msg.data.split('\0')
            posid = data[0]
            model = get_model(posid)
            order = eTree.XML(get_posot(model).orderPicture(posid=posid))
        ...
        with FiscalDataRepository(self.mbcontext) as fiscal_repository:  # type: fiscalpersistence.FiscalDataRepository
            ...
            try:
                ...
                if reprint:
                    ...
                    report = self._reprint_sat_or_nfce(fiscal_repository.get_xml_request(orderid), fiscal_repository.get_xml_response(orderid), params)
                    if report[0]:
                ...
                if self.nf_type in ("SAT", "MFE"):
                    ...
                    sat_used = sat_used[3:]
                    response_xml = str(fiscal_data.find("FiscalData"))
                    ...
                    report = self.fiscal_printer.print_fiscal(posid, order, tenders, fiscal_data.find("XmlRequest"), fiscal_data.find("FiscalData"), total_taxes, customer_name)
                else:
                    ...
                    idx_final = fiscal_data[0].find("</NFe") + 6
                    ...
                    if self.fiscal_printer and self.nf_type != "PAF":
                        report = self.fiscal_printer.print_fiscal(posid, order, tenders, request_xml, response_xml, total_taxes, customer_name)
                    else:
                        report = ""
                    ...
                    while True:
                        ...
                        for row in cursor:
                            ...
                            try:
                                ...
                                if not fiscal_data:
                                    raise Exception("Problemas ao tentar consultar a situação da nota na sefaz. "
                                ...
                                if c_stat in ("100", "150", "613"):  # 100 = ok; 613 = Chave de Acesso difere da existente em BD;
                                    if c_stat in ("613", ):
                                        tenders = fiscalrepository.get_tenders(order_id)

                                        try:
                                            logger.info("Regerando XML. Order %s " % order_id)
                                            order_picture = eTree.XML(base64.b64decode(order_picture + "=" * ((4 - len(order_picture) % 4) % 4)))
                                            envelopado, data_emissao, serie_nota, numero_nota = self.fiscal_processor.nfce_request_builder.build_request(pos_id, order_picture, tenders, False, None, None)
    ...
    def _handle_get_csts(self, msg):
        return True, self.fiscal_parameter_controller.get_all_products_csts()

    def _handle_get_base_reductions(self, msg):
    ...
    def _handle_sat_op_status_request(self, msg):
        ...
        try:
            ...
            logger.error("Erro buscando SAT %s", str(ex))
    ...
    def _parse_date_from_fiscal_screen(self, dates):
        try:
    ...
    def _find_fiscal_folders(self, start_date, end_date, msg):
        try:
            ...
            path = os.path.join(self.fiscal_processor.get_fiscal_sent_dir(), xml_type)
    ...
    def zip_folders(self, folder):
        try:
            ...
            try:
                os.remove(os.path.join(self.xml_path, zip_complete_name))
            except OSError:
                pass

            self.zip(folder, zip_name)
            shutil.move(zip_complete_name, self.xml_path)
    ...
    @staticmethod
    def _handle_sat_payment_status_request():
        return True, ''

    def _handle_cancel_order(self, msg):
        pos_id = msg.data
        model = get_model(pos_id)
        order = eTree.XML(get_posot(model).orderPicture(posid=pos_id))
        order_id = order.get('orderId')
        justificativa = "Teste de cancelamento"

        if self.nf_type in ("NFCE"):
    ...
    def _busca_parametros_sitef(order):
        # type: (ElementTree) -> (str, str, str)
        orderid = order.get("orderId")
        created_at = order.get("createdAt").replace("-", "").replace(":", "")
        data_fiscal = created_at[:8]
        hora_fiscal = created_at[9:15]

        return orderid, data_fiscal, hora_fiscal
    # END _busca_parametros_sitef

    def _reprint_sat_or_nfce(self, xml_request, xml_response, params):
        if not xml_request:
        ...
        if self.nf_type in ("SAT", "MFE"):
            ...
            return True, self.fiscal_printer.print_fiscal(params.get('posid'),
                                                          params.get('order'),
                                                          params.get('tenders'),
    ...
    def _get_xml_sat(self, xml_request):
        try:
    ...
    def _get_xml_nfce(self, xml_request, xml_response):
        try:
            ...
            xml_ws_version = int(float(xml_resp.find(".//retConsReciNFe").attrib['versao']))
            ...
            try:
                ...
                for row in cursor:
                    try:
                        ...
                        fiscal_data = self.fiscal_processor.nfce_request_builder.build_request(pos_id, order, tenders, is_in_contingence, dh_cont, x_just)
