...
class NfceRequestBuilder:
    ...
    def build_request(self, posid, order, tenders, contingencia, dh_contingencia, just_contingencia, paf=False):
        nfe, data_emissao, serie_nota, numero_nota = self._build_nfe(posid, order, tenders, contingencia, dh_contingencia, just_contingencia, paf)
    ...
    def _build_nfe(self, posid, order_xml, tenders, contingencia, dh_contingencia, just_contingencia, paf=False):
        # type: (str, eTree.ElementTree, list, bool, Optional[datetime], Optional[str], Optional[bool]) -> (str, str, unicode, int)
        try:
        ...
        logger.info("Order {} final {}. Valor {}".format(
                order.order_id,
                context.get("nfce_key")[30:],
                context.get("total_prod")
            )
        )
...
class NfceProcessor(FiscalProcessor):
    ...
    def request_fiscal(self, posid, order, tenders, paf=False):
        ...
        request, data_emissao, serie_nota, numero_nota,  = self.nfce_request_builder.build_request(posid, order, tenders, contingencia, dh_contingencia, just_contingencia, paf)  # type: str
        ...
        if not contingencia and self.nfce_autorizador:  # Em lojas PAF, nao envia para a sefaz, apenas salva o XML (autorizador = None)
            request_file = None
            ...
            try:
                ...
                request, response_consulta, tentativas = self.nfce_autorizador.autoriza_notas(request, 5)

                logger.info("Resposta da SEFAZ recebida para Order: %s" % order_id)

                logger.info("Order {} Enviada com a chave {}".format(
                    order_id.zfill(9),
                    nfe_key
                    )
                )
                ...
                request, data_emissao, serie_nota, numero_nota,  = self.nfce_request_builder.build_request(posid, order, tenders, contingencia, dh_contingencia, just_contingencia)  # type: str
                ...
                if response_consulta is not None:
                    if self.versao_ws == 4:
                        ...
                        xml_namespaces = lxmletree.fromstring(response_consulta).iter().next().nsmap
...
class NfceAutorizador(object):
    ...
    def _verifica_status_processamento_nfce(self, response_consulta_xml):
        # type: (eTree.ElementTree) -> None
    ...
    def _verifica_mesma_nota(self, response_xml):
        # type: (eTree.ElementTree) -> bool
        try:
...
class NfceConnectivityTester(object):
    ...
    def test_connectivity(self):
        if self.last_test and self.last_test > datetime.now() - timedelta(minutes=3):
            self.logger.info("Ainda não podemos validar a conexão com a sefaz. Ultimo teste efetuado a menos de 3 minutos")
                                        ...
                                        if stat in ("100", "204", "704", "539", "150", "635"):  # 100 = ok; 204, 539 = duplicidade (ja estava ok); 150 = ok, atrasada;
                                            ...
                                            if stat in ("539", ):
                                                ...
                                                try:
                                                    ...
                                                    envelopado, data_emissao, serie_nota, numero_nota = self.nfce_request_builder.build_request(order.pos_id, order_picture, tenders, False, None, None)
