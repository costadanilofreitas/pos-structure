...
function renderGroups(lineNumber, item, groups, currentGroup, maxItems, currentLevel) {
    ...
    if(item.productType !== "Option") {
        ...
        if(item.quantity > 0) {
            currentGroup.push((
                <div key={lineNumber + "-" + item.partCode + "." + item.context}>{padding + (item.quantity - item.defaultQuantity) + '\u00A0' + item.productName}</div>));
        } else {
            currentGroup.push((
                <div className="striked" key={lineNumber + "-" + item.partCode + "." + item.context}>{padding + item.productName}</div>));
        }
        nextLevel++;
    }
...
export {OrderItems, calculateColumns, calculateRows};
