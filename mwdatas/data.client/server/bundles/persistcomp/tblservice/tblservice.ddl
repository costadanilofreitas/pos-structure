...
CREATE TABLE TableType AS
    SELECT 1 AS TypeId, "Seat"  AS Descr UNION ALL
    SELECT 2 AS TypeId, "Tab"   AS Descr;


--
-- Table TableStatus:
--   Definitions of table types.
CREATE TABLE TableStatus AS
    SELECT 1 AS Status, "Available"       AS Descr UNION ALL
    SELECT 2 AS Status, "Waiting2BSeated" AS Descr UNION ALL
    SELECT 3 AS Status, "Seated"          AS Descr UNION ALL
    SELECT 4 AS Status, "InProgress"      AS Descr UNION ALL
    SELECT 5 AS Status, "Linked"          AS Descr UNION ALL
    SELECT 6 AS Status, "Totaled"         AS Descr UNION ALL
    SELECT 7 AS Status, "Closed"          AS Descr;
