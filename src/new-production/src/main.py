...
from production.model import NoBoxesConfiguredException, NoViewsConfiguredException
...
def main():
    ...
    if len(boxes_config) == 0:
        raise NoBoxesConfiguredException("No boxes configured")
    if len(views_config) == 0:
        raise NoViewsConfiguredException("No views configured")
    ...
    audit_logger_prod_states = config.find_values("Production.Manager.AuditLogProdStates") or []
