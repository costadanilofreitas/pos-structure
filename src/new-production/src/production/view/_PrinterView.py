    ...
    def generate_and_print_report(self, order):
        ...
        for _ in range(1, 5):
            try:
                ...
                self.exception("Exception trying to generate order id {}", order.order_id)
                time.sleep(1)
    ...
    def _print(self, text, attempt):
        try:
            ...
            if attempt >= MAX_RETRY - 1:
                self.exception("Printer unavailable - Failed to Print Report")
            else:
                self.debug("Error when trying to print on Printer {} in attempt {}".format(self.printer, attempt))
            return False
        except BaseException: # noqa
            self.exception("Exception when trying to print on Printer {} in attempt {}".format(self.printer, attempt))
            return False
