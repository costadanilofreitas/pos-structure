...
from messagebus import MessageBus, Message, DataType, TokenPriority, TokenCreator, Event
...
class ProductionManager(ProductionCommandProcessor, PublishScheduler, OrderChanger, StatisticUpdater):
    def __init__(self, production_repository, purge_interval, orders_life_time, message_bus, logger, audit_logger_prod_states):
        # type: (ProductionRepository, int, int, MessageBus, Logger, List) -> None
        self.production_repository = production_repository
        self.purge_interval = purge_interval
        self.orders_life_time = orders_life_time
        self.message_bus = message_bus
        self.logger = logger
        self.audit_logger_prod_states = audit_logger_prod_states
    ...
    def process_command(self, command):
        ...
        if isinstance(command, OrderModifiedCommand):
            self._handle_order_modified_command(command)
            return
            ...
            self._handle_purge_orders_command(command)
            return
            ...
            order = self._handle_change_prod_state_command(command)
            self.publish_auditlog_event(order)

        elif isinstance(command, UndoProductionCommand):
            order = self._handle_undo_production_command(command)

        elif isinstance(command, ChangePathCommand):
            self._handle_change_path_command(command)
    ...
    def _handle_change_path_command(self, command):
    ...
    def _handle_undo_production_command(self, command):
        try:
            view_undo_commands = self.undo_view_map[command.view]
            if len(view_undo_commands) == 0:
                return
        except KeyError:
            self.logger.info("No Undo Commands for the View: {}".format(command.view))
            return
    ...
    def _handle_change_prod_state_command(self, command):
    ...
    def _handle_purge_orders_command(self, command):
    ...
    def _handle_order_modified_command(self, command):
        order = command.order
    ...
    def remove_schedule_publish(self, order_id):
        # type: (int) -> None

        if order_id in self.waiters:
            self.waiters[order_id].deactivate()
    ...
    def set_all_items(self, item_view, item_save, item_setter):
        def get_diff_key(x):
        ...
        for son in item_save.items:
            if self.set_all_items(item_view, son, item_setter):
                were_change = True
        return were_change
    ...
    def change_items(self, order, item_changer):
        ...
        for view_item in order.items:
            for saved_item in saved_order.items:
                if self.set_all_items(view_item, saved_item, item_changer):
                    was_change = True
    ...
    def publish_auditlog_event(self, order):
        if order.prod_state not in self.audit_logger_prod_states:
            return

        order_id = str(order.order_id)
        try:
            production_order = eTree.fromstring(ProductionManager.get_order_xml(self, order.order_id))
            production_order_xml = eTree.tostring(production_order)
            data = """<ProductionOrderEvent posId="{0}" businessPeriod="{1}" orderId="{2}">{3}</ProductionOrderEvent>"""\
                .format(str(order.pos_id), str(order.business_day), order_id, production_order_xml)

            event = Event(subject="PRODUCTION_ORDER_EVENT", event_type="PRODUCTION_ORDER_TIMES", data=data)
            self.logger.info("Publishing AuditLog event: {} / {}".format(event.subject, event.event_type))
            self.message_bus.publish_event(event)
        except Exception as _:
            self.logger.exception("Error publishing auditlog event for orderId#{}".format(order_id))
