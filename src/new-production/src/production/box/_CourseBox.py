    ...
    def separate_product_in_courses(self, order):
        ...
        for item in order.items:
            if order.sale_type == "DELIVERY":
                ...
                if course_index == -1:
                    invalid_course_products.append(item)
                    continue
            course_products[course_index].append(item)

        for course_index in range(0, len(self.courses)):
            if len(course_products[course_index]) > 0:
                course_products[course_index].extend(invalid_course_products)
                break
        else:
            course_products[0].extend(invalid_course_products)

        for course_index in range(0, len(self.courses)):
            for item in course_products[course_index]:
        ...
        for index in range(0, len(courses)):
            ...
            for product in courses[index].products:
                if product.qty > 0:
                    all_course_items_canceled = False
                    break
