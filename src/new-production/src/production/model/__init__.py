...
from ._ProductionExceptions import TagNotFoundException, EventNotFoundException, ReportNameNotFoundException, \
    PrinterNameFoundException, ViewNameFoundException, CommandNotFoundException, BuilderNotFoundException, \
    ReprocessTimeNotFoundException, NoBoxesConfiguredException, NoViewsConfiguredException, \
    ReprocessInvalidTimeException
...
def is_item_done(item):
    if item.is_product() and item.qty > 0 and item.does_not_have_tag("done") and item.does_not_have_tag("no-cook"):
        return False
