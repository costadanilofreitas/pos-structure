...
class OrderService(object):
    ...
    def get_order_id(self, remote_order_id):
        ...
        if mw_order:
            ...
            raise Exception("The remote order {} does not exist".format(remote_order_id))
    ...
    def cancel_order(self, remote_order_id):
        ...
        if mw_order.status == MwOrderStatus.RECALLED:
            try:
                ...
                logger.info("Order RECALLED - OrderStatus: {1}; RemoteOrderId: {0}"
                            .format(remote_order_id, mw_order.status))
            except Exception as _:
                mw_order.status = MwOrderStatus.VOIDED
                logger.exception("Order RECALLED - ERROR in Order Picture - VOIDING. RemoteOrderId: {}"
