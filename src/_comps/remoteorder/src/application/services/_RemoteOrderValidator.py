...
class RemoteOrderValidator(object):
    def __init__(self, composition_tree_builder, composition_validator, order_price_calculator, warning_emitter, validate_delivery_price, validate_delivery_value):
        # type: (CompositionTreeBuilder, CompositionTreeValidator, OrderPriceCalculator, WarningEmitter, bool, float) -> None
    ...
    def validate_order(self, remote_order, unavailable_items=None):
        # type: (RemoteOrder, List[unicode]) -> List[CompositionTree]
        ...
        if not remote_order.items:
            raise OrderValidationException(OrderValidationError.OrderWithNoItens, "The remote order has no itens")
    ...
    def already_processed(self, remote_order_id, order_pict):
    ...
    def _find_in_salelines(self, composition_tree, sale_lines, current_line_number=None):
        item_found = False
        current_line_number = None
        for line in sale_lines:
            line_number = line.get('lineNumber')
            if line.get('partCode') == composition_tree.product.part_code and current_line_number in [None, line_number]:
                # found product, continue checking sons, if any
                item_found = True
                for son in composition_tree.sons:
                    try:
                        self._find_in_salelines(son, sale_lines, line_number)
                    except Exception:
                        item_found = False
    ...
    def validate_order_contains(self, order_trees, order_pict):
        ...
        for composition_tree in order_trees:
            self._find_in_salelines(composition_tree, sale_lines)
    ...
    def validate_order_price(self, remote_order, order_trees):
        ...
        if total_price != remote_order.sub_total:
