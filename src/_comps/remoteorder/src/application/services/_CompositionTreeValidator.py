...
from application.compositiontree import CompositionTree, ProductNode, CompositionType
...
class CompositionTreeValidator(object):
    ...
    def _check_min_quantity(self, new_composition_tree, composition_tree):
        # type: (CompositionTree, CompositionTree) -> None

        product = composition_tree.product
        new_product = new_composition_tree.product
        if product.min_qty is not None and ((new_product.current_qty or 0) < product.min_qty):
            new_product.current_qty = product.min_qty

        if composition_tree.sons is not None:
            for son in composition_tree.sons:
                new_composition_son = next((new_son for new_son in new_composition_tree.sons if new_son.product.part_code == son.product.part_code), None)
                if new_composition_son is None:
                    if son.product.min_qty <= 0:
                        continue

                    if composition_tree.product.type == CompositionType.Option:
                        message = "The min quantity of the item {0}->{1} was not reached"
                        message = message.format(composition_tree.product.part_code, son.product.part_code)
                        raise InvalidQuantityException(OrderValidationError.MinQuantityNotReached, message)

                    if son.product.type == CompositionType.Product or son.product.type == CompositionType.Combo:
                        created_son = self.create_product(new_composition_tree, son, son.product.min_qty, son.product.price, False)
                        new_composition_tree.sons.append(created_son)
                        self._check_min_quantity(created_son, son)

                    elif son.product.type == CompositionType.Option:
                        # Criamos o Option
                        created_son = self.create_product(new_composition_tree, son, son.product.min_qty, son.product.price, True)
                        new_composition_tree.sons.append(created_son)

                        # Adicionamos, no Options, todos os produtos que tem default quantity
                        for grand_son in son.sons:
                            if grand_son.product.default_qty > 0:
                                create_quantity = grand_son.product.default_qty
                                created_grand_son = self.create_product(created_son, grand_son, create_quantity, grand_son.product.price, False)
                                created_son.sons.append(created_grand_son)
                                self._check_min_quantity(created_grand_son, grand_son)
                        self._check_min_quantity(created_son, son)

                elif (new_composition_son.product.current_qty or 0) > 0:
                    self._check_min_quantity(new_composition_son, son)

    def _position_trees(self, new_composition_tree, composition_tree, part_code):
        # type: (CompositionTree, CompositionTree, str) -> Union[Tuple[CompositionTree, CompositionTree], Tuple[None, None]]
