...
def main():
    ...
    use_delivery_fee = (config.find_value("RemoteOrder.UseDeliveryFee") or "true").lower() == "true"
    ...
    remote_order_taker = RemoteOrderTaker(pos_id, order_taker_wrapper, order_item_creator, order_repository,
                                          api_product_repository, delivery_fee_part_code)
    ...
    logistic_service = LogisticService(mb_context, pos_id, order_service, store_id, config)
    ...
    message_handler.subscribe_queue_events([ListenedEvents.LogisticOrderConfirm, ListenedEvents.Ping,
                                            ListenedEvents.RupturaDataUpdated, ListenedEvents.PosOrderProducedAck,
                                            ListenedEvents.RuptureAck, ListenedEvents.LogisticSearching,
                                            ListenedEvents.LogisticFound, ListenedEvents.LogisticNotFound,
                                            ListenedEvents.LogisticCanceled, ListenedEvents.PosLogisticConfirmAck,
                                            ListenedEvents.LogisticFinished, ListenedEvents.PosOrderConfirmAck])
