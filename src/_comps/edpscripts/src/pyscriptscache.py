...
class Cache:
    ...
    def get_best_price(self, context, part_code, pod_type="EI",forced_pricelist=''):
        # type: (str, str, str, str) -> Optional[dict]
        logger.info("Forced PriceList %s" % forced_pricelist)
        best_price, _ = self._get_best_price_and_key(context, part_code, pod_type, forced_pricelist=forced_pricelist)
        return best_price

    def get_best_price_key(self, context, part_code, pod_type="EI", forced_pricelist=''):
        # type: (str, str, str, str) -> Optional[dict]
        logger.info("Forced PriceList %s" % forced_pricelist)
        _, price_key = self._get_best_price_and_key(context, part_code, pod_type, forced_pricelist=forced_pricelist)
        return price_key

    def _get_best_price_and_key(self, context, part_code, pod_type, forced_pricelist=''):
        best_price = None
        best_price_key = None
        logger.info("Forced PriceList %s" % forced_pricelist)

        if pod_type == "DL":
            best_price, best_price_key = self._find_best_price_and_key_by_pod_type(context, part_code, "DL", forced_pricelist=forced_pricelist)
        if best_price is None and best_price_key is None:
            best_price, best_price_key = self._find_best_price_and_key_by_pod_type(context, part_code, "EI", forced_pricelist=forced_pricelist)

        return best_price, best_price_key

    def _find_best_price_and_key_by_pod_type(self, current_context, part_code, pod_type, forced_pricelist=''):
        all_contexts = Cache._get_all_product_contexts(current_context)
        logger.info("Forced PriceList %s" % forced_pricelist)

        for context in all_contexts:
            price = self._get_price_in_context(context, part_code, pod_type, forced_pricelist=forced_pricelist)
            price_key = self._get_price_key_in_context(context, part_code, pod_type, forced_pricelist=forced_pricelist)
    ...
    def _get_price_in_context(self, context, part_code, pod_type, forced_pricelist=''):
        key = (part_code, context)
        logger.info("Forced PriceList %s" % forced_pricelist)
        if forced_pricelist:
            pod_type = forced_pricelist
    ...
    def _get_price_key_in_context(self, context, part_code, pod_type, forced_pricelist=''):
        key = (part_code, context)
        if forced_pricelist:
            pod_type = forced_pricelist
    ...
    def _product_prices_cache_renewer(self):
        conn = None
        try:
            lists = get_valid_pricelists(self.mbcontext)
            all_prices = {}
            conn = persistence.Driver().open(self.mbcontext)
            for price_list in lists:
                all_prices[price_list] = Cache._query_all_prices_by_price_list(conn, price_list)
    ...
    def _price_key_cache_renewer(self):
        ...
        try:
            ...
            conn = persistence.Driver().open(self.mbcontext)
            for price_list in lists:
                all_prices_key[price_list] = Cache._query_all_price_keys_by_price_list(conn, price_list)
