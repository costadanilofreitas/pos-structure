...
class UserHelper:
    ...
    def get_all_users(self):
        # type: () -> dict
        return {x.get_entry(0): (x.get_entry(1), x.get_entry(2), x.get_entry(3), x.get_entry(4))
                for x in self.conn.select("""select UserId, LongName, Level, Status from Users""")}
    ...
    def change_pass(self, userid, passwd):
        ...
        calc.update(passwd)
        the_passwd = calc.hexdigest()
        return self.conn.query("update Users set Password = '%s' where UserId = '%s'" % (the_passwd, userid))
