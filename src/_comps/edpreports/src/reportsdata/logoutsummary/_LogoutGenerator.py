...
from reports_app.logoutsummary.dto import LogoutSummaryInfo  # noqa
from reports_app.logoutsummary.dto import LogoutSummaryDto
from reports_app.logoutsummary.dto import LogoutSummaryHeaderDto
from reports_app.logoutsummary.dto import LogoutSummaryBodyDto
from reports_app.logoutsummary.dto import CashReportBodyLineDto
from reports_app.logoutsummary.dto import OrderInfo


class LogoutGenerator(Generator):
    def __init__(self, account_repository, order_repository, logout_summary_info):
        # type: (AccountRepository, OrderRepository, LogoutSummaryInfo) -> ()
        self.logout_summary_info = logout_summary_info
        self.account_repository = account_repository
        self.order_repository = order_repository

    def generate_data(self):
        # type: (LogoutSummaryInfo) -> LogoutSummaryDto
        logout_summary_header_dto = LogoutSummaryHeaderDto(
            self.logout_summary_info.pos_id,
            self.logout_summary_info.store_id,
            self.logout_summary_info.operator_id,
            self.logout_summary_info.business_date
        )
        logout_summary_body_dto = self.generate_body()

        return LogoutSummaryDto(logout_summary_header_dto, logout_summary_body_dto)
    ...
    def generate_body(self):
        ...
        return LogoutSummaryBodyDto(
            self.logout_summary_info.authorizer,
            self.logout_summary_info.login_time,
            [initial_balance,
             sales,
             voided_orders,
             discounts,
    ...
    def get_order_db_info(self):
        paid_orders = self.order_repository.get_paid_orders_by_session_id(
            self.logout_summary_info.session_id)
