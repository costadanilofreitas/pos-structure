...
class AccountRepository(BaseRepository):
    ...
    def get_transfers_by_real_date(self, initial_date, end_date, operator_id, report_pos, session_id=None):
        # type: (datetime, datetime, Optional[unicode]) -> List[Transfer]
        return self._get_transfer(self.TypeRealDate, initial_date, end_date, operator_id, session_id, report_pos)
    ...
    def _get_transfer(self, report_type, initial_date, end_date, operator_id, session_id, report_pos):
        ...
        def inner_func(conn):
            ...
            current_session_id = session_id if session_id is not None else ''

            if report_type == self.TypeRealDate:
                query = self._TransfersByRealDate
                query = query.format(initial_date.strftime("%Y-%m-%d"), end_date.strftime("%Y-%m-%d"), current_session_id)
    ...
    _TransfersByRealDate = "select Type, Amount from Transfer where strftime('%Y-%m-%d', Timestamp, 'localtime') >= '{0}' and strftime('%Y-%m-%d', Timestamp, 'localtime') <= '{1}' and SessionId like '%{2}%'"

    _TransfersByBusinessPeriod = "select Type, Amount from Transfer where Period >= '{0}' and Period <= '{1}' and SessionId like '%{2}%'"
