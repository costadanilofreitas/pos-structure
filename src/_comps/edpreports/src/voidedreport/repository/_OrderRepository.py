...
"""select o.OrderId, coalesce(vrs.Value, 'Nao Populado') as VoidReason, o.SessionId, aut.Value as AuthenticationUser, osh.OrderDate as Timestamp, coalesce(o.TotalGross, 0) as TotalGross
    from Orders o
    left join (
    ...
    where o.StateId = 4 and osh.OrderDate is not null and osh.StateId = 4"""

    _VoidedOrdersByBusinessPeriodQuery = \
"""select o.OrderId, coalesce(vrs.Value, 'Nao Populado') as VoidReason, o.SessionId, aut.Value as AuthenticationUser, osh.Timestamp as Timestamp, coalesce(o.TotalGross, 0) as TotalGross
    from Orders o
	left join OrderStateHistory osh on osh.OrderId = o.OrderId
	left join OrderCustomProperties vrs on vrs.OrderId = o.OrderId and vrs.Key = 'VOID_REASON_DESCR'
	left join OrderCustomProperties aut on aut.OrderId = o.OrderId and aut.Key = 'AUTHENTICATION_USER'
	where o.BusinessPeriod >= '{0}' and o.BusinessPeriod <= '{1}' and o.StateId = 4 and osh.Timestamp is not null and osh.StateId = 4"""
...
"""select o.OrderId, coalesce(vrs.Value, 'Nao Populado') as VoidReason, o.SessionId, aut.Value as AuthenticationUser, osh.Timestamp as Timestamp, coalesce(o.TotalGross, 0) as TotalGross
	...
	left join OrderCustomProperties vrs on vrs.OrderId = o.OrderId and vrs.Key = 'VOID_REASON_DESCR'
	left join OrderCustomProperties aut on aut.OrderId = o.OrderId and aut.Key = 'AUTHENTICATION_USER'
	where o.SessionId = '{0}' and o.StateId = 4 and osh.Timestamp is not null and osh.StateId = 4"""
