...
class RecalledOrderStampBox(OrderChangerProductionBox):
    ...
    def check_item_recalled(self, order, item):
        ...
        if order.prod_state not in (ProdStates.SERVED, ProdStates.DELIVERED):
            if any(s.prod_state in (ProdStates.SERVED, ProdStates.DELIVERED) for s in order.prod_state_history):
                order.recalled = True
                return order
