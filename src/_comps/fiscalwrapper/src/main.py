...
from nfce import NfceProcessor, NfceRequestBuilder, NfceContingencia, NfceAutorizador, NfceConnectivityTester, NfceSituationChecker
...
def main():
    ...
    end_logradouro = _limit_max_length(read_swconfig(mbcontext, "Store.EndLogradouro"), 60)
    end_numero = read_swconfig(mbcontext, "Store.EndNumero").strip() or "S/N"
    end_compl = (read_swconfig(mbcontext, "Store.EndCompl") or "").strip()
    ...
    cnpj_contribuinte = config.find_value("FiscalWrapper.CnpjContribuinte")
    inscr_estadual = config.find_value("FiscalWrapper.InscEstadual")
