...
from msgbus import MBException, FM_STRING, FM_PARAM, TK_SYS_ACK, SE_NOTFOUND, MBEasyContext
...
from satbuilder import CfeBuilder
...
class SatRequestBuilder:
    ...
    def build_request(self, order_xml):
        # type: (eTree) -> unicode
...
class SatServiceFinder:
    def __init__(self, mbcontext, max_sat_number, sleep_time):
        # type: (MBEasyContext, int, int) -> None
        self.mbcontext = mbcontext
        self.max_sat_number = max_sat_number
        self.sleep_time = sleep_time

        self.search_thread = None  # type: Optional[Thread]
    ...
    def find_and_lock_sat_service(self, order_id):
        ...
        with self.available_sats_lock:
            if not self.available_sats:
                ...
                raise Exception("Nenhum SAT encontrado")
        ...
        for i in range(0, self.max_sat_number + 1):
            ...
            try:
                ...
                if ret.data == "OK":
                    ...
                    try:
                        set_custom(i, 'SAT_STATUS', '1') if i != 0 else None
                    except Exception:
                        pass
                else:
                    logger.info("{0} not available: {1}".format(sat_service_name, ret.data))
                    try:
                        set_custom(i, 'SAT_STATUS', '2') if i != 0 else None
                    except Exception:
                        pass
            except MBException as e:
                try:
                    set_custom(i, 'SAT_STATUS', '3') if i != 0 else None
                except Exception:
                    pass
...
class SatProcessor(FiscalProcessor):
    ...
    def request_fiscal(self, pos_id, order, tenders, paf=False):
        # type: (str, eTree, List[Tender], Optional[bool]) -> eTree.Element
        request = self.sat_request_builder.build_request(order)

        sat_service_name = self.sat_service_finder.find_and_lock_sat_service(order.get("orderId"))

        self.last_sat_used[pos_id] = sat_service_name
        ...
        try:
            ...
            req = '\0'.join([pos_id, order.attrib["orderId"], request.encode('utf-8')])
            ...
            if msg.token == TK_SYS_ACK:
                ...
                logger.debug("OrderId: %s; request size: %d" % (order_id, len(padded)))
                base64_xml = base64.b64decode(padded)
                logger.debug("OrderId: %s; base64_xml: %s" % (order_id, base64_xml))
                ...
                sysactions.get_posot(sysactions.get_model(pos_id)).setOrderCustomProperty("FISCALIZATION_DATE", emissao_date.strftime("%Y-%m-%dT%H:%M:%S"))

                self.sat_service_finder.unlock_sat_service(sat_service_name)

                cfe_code = ret_xml.find("FiscalData").find("satkey").text

                response_file_dir = os.path.join(dir_nota, "S{0}_{1}_{2}_response_pos{3}_CFe{4}.xml".format(sat_service_name[-2:], num_nota_cfe, order_id, str(pos_id).zfill(2), cfe_code))
                ...
                request_erro = open(os.path.join(dir_erro, "{0}_request_{1}_pos{2}_order{3}_erro.xml".format(order_id, sat_service_name, str(pos_id).zfill(2), order.get("orderId"))), "w+")
                request_erro.write(str(request))
                request_erro.close()
    ...
    def build_cancel_xml(self, order_request):
        ...
        cnpj = self.sat_request_builder.cnpj_sw
