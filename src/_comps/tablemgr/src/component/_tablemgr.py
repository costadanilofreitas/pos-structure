...
def create_order(pos_id, table, storeorder=False, blk_notify=False, pricelist="EI", sessionid="", additionalinfo="", multiorderid="", ordertype="SALE", saletype="EAT_IN", ordersubtype="", customProperties={}):
    params = ("1" if blk_notify else "0", pos_id, pricelist, "POS%02d" % int(pos_id), "", table["businessPeriod"].replace('-', ''), "TS", sessionid, additionalinfo, multiorderid, ordertype, saletype, ordersubtype)
    ...
    try:
        ...
        if ex.get_code() == "8":  # There is an Open Order in POS
            ...
            return create_order(pos_id, table, storeorder, blk_notify, pricelist, sessionid, additionalinfo, multiorderid, ordertype, saletype, ordersubtype, customProperties)
...
def handle_slice_service(argv, msg=None):
    ...
    try:
        ...
        new_order_id = create_order(
            ...
            customProperties={"SOURCE_SERVICE_ID": srctable["serviceId"]}
        )
...
def handle_total_service(argv, msg=None):
    ...
    splitobj = {
        ...
        "splitOrderId": create_order(
            ...
            customProperties={"SPLIT_KEY": DEFAULT_SPLIT_KEY}
        )
    }
