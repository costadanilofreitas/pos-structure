...
from mw.common import persistence
import requests
...
class HyperVisorDriver(object):
    ...
    TK_HV_COMPSTART = TokenCreator.create_token(TokenPriority.low, MSGGRP_HV, "1")
    TK_HV_COMPKILL = TokenCreator.create_token(TokenPriority.high, MSGGRP_HV, "2")
    TK_HV_COMPLIST = TokenCreator.create_token(TokenPriority.low, MSGGRP_HV, "3")

    def __init__(self, mw_app_url):
        self.mwapp_url = mw_app_url
        self.services_url = mw_app_url + "/mwapp/services"
    ...
    def _get_service_list_xml(self, service_name):
        url = self.services_url + "/HyperVisor/HyperVisor/?token=" + get_token_hexa_string(HyperVisorDriver.TK_HV_COMPLIST) + "&format=xml&timeout=-1&isBase64=false"
    ...
    def _kill_service_by_pid(self, pid):
        url = self.services_url + "/HyperVisor/HyperVisor?token=" + get_token_hexa_string(HyperVisorDriver.TK_HV_COMPKILL) + "&format=string&timeout=-1&isBase64=false"
        response = requests.post(url, data=pid, timeout=10.0)
        HyperVisorDriver._check_response_and_token_errors(url, response)

    def _start_service_by_service_name(self, service_name):
        url = self.services_url + "/HyperVisor/HyperVisor?token=" + get_token_hexa_string(HyperVisorDriver.TK_HV_COMPSTART) + "&format=string&timeout=-1&isBase64=false"
