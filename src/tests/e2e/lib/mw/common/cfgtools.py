# Embedded file name: C:\Program Files\OpenSSH\gitlabci\mwapp\src\wrappers\python\cfgtools.py
"""
    ...
    def _write(self, out, level):
        ...
        if self.values:
            ...
            for val in self.values:
                out.write('%s<%s>%s</%s>\n' % ('\t' * level,
                 typestr,
                 saxutils.escape(val.encode('UTF-8')),
                 typestr))
