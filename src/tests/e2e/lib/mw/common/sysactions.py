# Embedded file name: C:\Program Files\OpenSSH\gitlabci\mwapp\src\wrappers\python\sysactions.py
"""
Module sysactions
...
from msgbus import TK_SYS_NAK, TK_POS_SETDEFSERVICE, TK_SYS_ACK, TK_POS_GETCONFIG, TK_POS_GETPOSLIST, TK_MSR_REQUESTSWIPE, TK_POS_SETMAINSCREEN, TK_POS_SETSCREEN, TK_POS_GETMODEL, TK_LDISP_WRITE, TK_POS_SETINFOMSG, TK_POS_SHOWDIALOG, TK_POS_DIALOGRESP, TK_REPORT_GENERATE, TK_POS_UPDATEDIALOG, TK_PRN_PRINT, TK_STORECFG_GETFULL, TK_POS_MODELSETCUSTOM, TK_STORECFG_GET, TK_I18N_TRANSLATE, TK_L10N_TOCURRENCY, TK_USERCTRL_GETINFO, TK_USERCTRL_GETINFO_BADGE, TK_USERCTRL_AUTHENTICATE, TK_PERSIST_QUERYEXEC, TK_EVT_EVENT, TK_HV_LISTHVS, FM_STRING, FM_PARAM, FM_XML, MBTimeout, MBException
from posot import OrderTaker
__all__ = ('StopAction', 'AuthenticationFailed', 'action', 'send_message', 'change_mainscreen', 'change_screen', 'get_model', 'get_podtype', 'get_posfunction', 'get_business_period', 'check_genesis_error', 'get_operator_session', 'get_posot', 'get_used_service', 'write_ldisplay', 'is_day_opened', 'is_day_blocked', 'is_drawer_opened', 'is_operator_logged', 'has_operator_opened', 'get_current_operator', 'has_current_order', 'has_open_options', 'get_current_order', 'check_drawer', 'check_business_day', 'check_operator_logged', 'check_current_order', 'show_info_message', 'show_any_dialog', 'show_messagebox', 'show_confirmation', 'show_listbox', 'show_keyboard', 'show_form', 'show_print_preview', 'show_ppview', 'remove_xml_prolog', 'show_order_preview', 'show_text_preview', 'close_asynch_dialog', 'generate_report', 'print_text', 'print_report', 'set_custom', 'get_custom', 'clear_custom', 'translate_message', 'format_date', 'format_amount', 'get_last_order', 'get_line_product_name', 'get_clearOptionsInfo', 'can_void_line', 'get_user_information', 'authenticate_user', 'is_valid_date', 'calculate_giftcards_amount', 'get_custom_params', 'get_dimension', 'get_pricelist', 'check_main_screen', 'get_storewide_config', 'on_before_total', 'assert_order_discounts', 'get_authorization', 'read_msr', 'get_poslist', 'get_cfg', 'get_tender_types', 'get_tender_type', 'show_custom_dialog', 'is_valid_datetime', 'encode_string', 'decode_string', 'read_scanner', 'list_hv_nodes', 'ha_get_context', 'ha_switch_servers', 'update_any_dialog', 'update_messagebox', 'update_confirmation', 'order_has_items')
...
def get_poslist():
    ...
    if msg.token == TK_SYS_NAK or not msg.data:
        time.sleep(2)
        msg = send_message('PosController', TK_POS_GETPOSLIST)
    poslist = tuple((int(posid) for posid in msg.data.split('\x00')))
    return poslist
...
def read_msr(logicalname, timeout = 60000, dialogid = -1, params = []):
    ...
    try:
        send_message(logicalname, TK_MSR_REQUESTSWIPE, FM_STRING, '\x00'.join([str(timeout)] + params))
    except:
...
def change_mainscreen(posid, screenid, persist = False):
    ...
    """
    send_message('POS%d' % int(posid), TK_POS_SETMAINSCREEN, FM_PARAM, '%s\x00%s\x00%s' % (posid, screenid, 'true' if persist else 'false'))
...
def change_screen(posid, screenid, previousid = ''):
    ...
    """
    send_message('POS%d' % int(posid), TK_POS_SETSCREEN, FM_PARAM, '%s\x00%s\x00%s' % (posid, screenid, previousid))
    ...
    if service:
        try:
            if marquee is None:
                ...
                reqbuf = '%s\x00%s' % (text, 'start' if marquee else 'stop')
...
def has_open_options(posid, model, show_message = False):
    ...
    if open_options:
        if show_message:
            product_name = get_line_product_name(model, int(open_options[0]['lineNumber']))
...
def show_info_message(posid, message, timeout = 5000, msgtype = 'info'):
    ...
    """
    send_message('POS%d' % int(posid), TK_POS_SETINFOMSG, FM_PARAM, '%s\x00%s\x00%d\x00%s' % (posid,
     message,
     timeout,
     msgtype))
...
def show_any_dialog(posid, dlgtype, message, title, infotext, buttons, icon, timeout, mask, defvalue, minvalue, maxvalue, contents, asynch, linefeed = '\\', focus = True, custom_dlg_id = '', custom_dlg_timeout = '0'):
    ...
    dlgtype, message, title, infotext, buttons, icon, mask, defvalue, minvalue, maxvalue, linefeed, focus, custom_dlg_id, custom_dlg_timeout = map(saxutils.quoteattr, map(str, (dlgtype,
     message,
     title,
     infotext,
     buttons,
    ...
    etree.XML(xml)
    params = '%(posid)s\x00%(xml)s\x00%(timeout)s\x00%(asynch)s' % locals()
...
def update_any_dialog(posid, dlgid, dlgtype, message, title, infotext, buttons, icon, timeout, mask, defvalue, minvalue, maxvalue, contents, linefeed = '\\', focus = True, custom_dlg_id = '', custom_dlg_timeout = '0'):
    ...
    xml = '<?xml version="1.0" encoding="UTF-8"?>\n    <DialogBox type=%(dlgtype)s title=%(title)s message=%(message)s info=%(infotext)s\n    btn=%(buttons)s icon=%(icon)s mask=%(mask)s default=%(defvalue)s\n    min=%(minvalue)s max=%(maxvalue)s lineFeed=%(linefeed)s focus=%(focus)s custom_dlg_id=%(custom_dlg_id)s custom_dlg_timeout=%(custom_dlg_timeout)s>\n        %(contents_tag)s\n    </DialogBox>\n    ' % locals()
    etree.XML(xml)
    params = '%(posid)s\x00%(dlgid)s\x00%(xml)s\x00%(timeout)s\x00true' % locals()
...
def show_custom_dialog(posid, custom_dlg_id, custom_dlg_timeout = '0'):
    ...
    if timeout == 0:
        ...
        timeout += 30000
    message, title, infotext, buttons, icon, mask, defvalue, minvalue, maxvalue, contents, asynch, linefeed, focus = ('',
     '',
     '',
     '',
     '',
...
def show_keyboard(posid, message, title = '$INPUT_DATA', infotext = '$PRESS_OK_TO_CONTINUE', buttons = '$OK|$CANCEL', icon = 'info', timeout = 60000, mask = '', is_password = False, defvalue = '', minvalue = '', maxvalue = '', numpad = False, nopad = False, noinput = False, asynch = False):
...
show_ppview = show_print_preview
...
def generate_report(name, *params):
    ...
    msgparams = '\x00'.join(map(str, msgparams))
        ...
        while printer_service:
            ...
            if not msg or msg.token == TK_SYS_NAK:
                ...
                if not force_printer:
                    msg = send_message('POS%d' % int(posid), TK_POS_SETDEFSERVICE, FM_PARAM, '%s\x00printer\x00%s' % (posid, printername))
...
def set_custom(posid, key, value, persist = False):
    ...
    res = send_message('POS%d' % int(posid), TK_POS_MODELSETCUSTOM, FM_PARAM, '%s\x00%s\x00%s\x00%s' % (posid,
     key,
     value,
     persist))
...
def translate_message(model, message_key, *params):
    ...
    data = '%s\x00%s\x00%s' % (lang_name, message_key, '\x00'.join(params))
...
def format_amount(model, amount, addsymbol = True):
    ...
    """
    lang_name = str(model.find('Language').get('name'))
    data = '%s\x00%s\x00%s' % (lang_name, amount, 'true' if addsymbol else 'false')
...
def get_last_order(model):
    ...
    if not orders:
        return None
    elif len(orders) == 1 or orders[0]['state'] in ('PAID', 'VOIDED', 'ABANDONED', 'STORED'):
        return orders[0]
    else:
        return orders[1]
...
def authenticate_user(userid, password, min_level = None):
    ...
    """
    msg = send_message('UserControl', TK_USERCTRL_AUTHENTICATE, FM_PARAM, '%s\x00%s\x00' % (userid, password))
    if msg.token == TK_SYS_NAK:
        raise AuthenticationFailed('INVALID_USER_OR_PASSWORD')
    level, username, longusername = msg.data.split('\x00')[:3]
    if min_level is not None and int(level) < int(min_level):
        raise AuthenticationFailed('ACCESS_DENIED')
    return {'Level': level,
     'UserName': username,
     'LongName': longusername}
...
def get_storewide_config(parameter, defval = None, array = False):
    ...
    if msg.token == TK_SYS_ACK:
        if array:
            if msg.size:
                return msg.data.split('\x00')
            return defval
        else:
            return msg.data.split('\x00')[0] or defval
    return defval
...
def list_hv_nodes():
    ...
    if msg and msg.token == TK_SYS_ACK:
        return [ h.attrib for h in etree.XML(msg.data).findall('hypervisor') ]
    return []
...
def ha_get_context(posid):
    ...
    for pos_node in pos_nodes:
        if int(pos_node.upper().replace('POS', '')) == int(posid):
            msg = mbcontext.MB_EasySendMessage('HA_{0}'.format(pos_node), TK_EVT_EVENT, data='\x00'.join([pos_node, 'GET_CONTEXT', 'HAMANAGER']))
...
def ha_switch_servers(posid):
    ...
    if context and context['slave'] and 'secs_since_last_update' in context['slave']:
        if context['slave']['secs_since_last_update'] <= 5:
            msg = mbcontext.MB_EasyEvtSend('SWITCH_SERVERS_REQUEST', 'HAMANAGER', json.dumps({'source': context['node_name'],
             'slave': context['slave']}))
        else:
