# Embedded file name: C:\Program Files\OpenSSH\gitlabci\mwapp\src\wrappers\python\kiosk_common.py
try:
...
def read_swconfig(key, mbcontext):
    ...
    rmsg = mbcontext.MB_EasySendMessage('StoreWideConfig', token=msgbus.TK_STORECFG_GET, format=msgbus.FM_PARAM, data=key)
...
class KioskSession(object):
    ...
    def get_session(self, access_token):
        query = "SELECT id, udid, posid, access_token, offline_session, fulfillment_method,                 created_on, user_details, identify_details                 FROM KioskSession WHERE access_token = '{0}'".format(access_token)
        ...
        if cursor.rows() > 0:
            ...
            if identify_details:
                ...
                identify_details = {}
            session = {'id': int(cursor.get_row(0).get_entry(0)),
    ...
    def create(self, udid, access_token = None, user_details = '', identify_details = ''):
        ...
        try:
            ...
            if posid_match:
                ...
                pos_id = 1
            conn = self.driver.open(self.mbcontext)
            query = "INSERT INTO KioskSession (udid, posid, access_token, offline_session, user_details, identify_details)                 VALUES ('{0}', '{1}', '{2}', '{3}', '{4}', '{5}')".format(udid, pos_id, self.access_token, offline_session, user_details, identify_details)
            conn.query(query)
            conn.close()
        except:
...
class Purchase(object):
        ...
        if self.purchase_data['type'] == 'COMBO':
            for _purchase in self.purchase_data['purchases']:
                ...
                for customization in _purchase['customizations']:
                    ...
                    elif 'ingredient' in customization.keys():
    ...
    def _store_request_params(self, pos_id, purchase_id, purchase_data, online_purchase_id):
        ...
        self.mbcontext.MB_EasySendMessage('order_buffer', token=msgbus.TK_BUF_BUFFER_PURCHASE, format=msgbus.FM_PARAM, data=json.dumps(data))
    ...
    def create_line(self, post_params = None):
        ...
        if post_params:
            ...
            sale = ot.doSale(self.pos_id, product, self.price_list, verifyOption=False, aftertotal='0')
        if not sale:
            self.logger.error('Something wrong happend during sale of {0}'.format(product))
        else:
            self.logger.debug('Sale of {0} finished.'.format(product))
    ...
    def delete_line(self, order_id, line_number):
        ...
        conn = self.driver.open(self.mbcontext, dbname=str(self.pos_id))
        conn.query(update)
        delete_line_query = "DELETE FROM PurchaseItems\n            WHERE pos_id = '{0}' AND purchase_id = '{1}0000{2}'".format(self.pos_id, order_id, line_number)
        conn.query(delete_line_query)
        delete_discount_query = "DELETE FROM OrderDiscounts\n            WHERE purchase_id = '{0}0000{1}'".format(order_id, line_number)
    ...
    def update_line(self, purchase_id, new_purchase_data):
        ...
        main_product_query = 'SELECT PartCode FROM CurrentOrderItem WHERE             Level = 0 AND OrderId = {0} AND LineNumber = {1}'.format(order_id, line_number)
        conn = self.driver.open(self.mbcontext, dbname=str(self.pos_id))
    ...
    def get_order_price(self):
        ...
        msg = self.mbcontext.MB_EasySendMessage('ORDERMGR%s' % self.pos_id, msgbus.TK_SLCTRL_OMGR_ORDERPICT, msgbus.FM_PARAM, reqbuf)
        if any((x in msg.data for x in ('state="VOIDED"', 'state="ABANDON"'))):
            ...
            for method in fulfillment_methods:
                if discount_amount:
                    ...
                    pricing = {'subtotalWithoutDiscount': round(totalAmount, 2),
                    ...
                    pricing = {'fulfillmentMethodId': method,
                     'taxValue': round(taxTotal, 2),
    ...
    def set_panera_id(self, panera_id):
        ...
        if cursor.rows() > 0:
            ...
            query = "INSERT INTO Purchases (purchase_id, panera_id)                 VALUES ('{0}', '{1}')".format(order_id, panera_id)
        conn.query(query)
        conn.close()
    ...
    def panera_id(self):
        ...
        query = "SELECT panera_id FROM Purchases             WHERE purchase_id = '{0}'".format(purchase_id)
        conn = self.driver.open(self.mbcontext, dbname=str(self.pos_id))
        cursor = conn.select(query)
        if cursor.rows() > 0:
            conn.close()
...
class Combo(object):
    ...
    def response(self):
        response = {'type': self.type,
         ...
         'purchases': [],
         'product': {'id': self.purchase.main_product},
