...
class IntegrationService(object):
    ...
    def _save_integration_status(self, order_id, delivery_status, logistic_status):
        with self.concurrent_events_lock:
            ...
            current_logistic_status = self.order_service.get_order_custom_property(order_id, key)
            if not current_logistic_status:
                self.order_service.set_order_custom_property(order_id, key, logistic_status)
