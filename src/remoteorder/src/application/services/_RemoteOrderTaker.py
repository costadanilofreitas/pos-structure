...
class RemoteOrderTaker(object):
    def __init__(self, pos_id, order_taker_wrapper, order_item_creator, order_repository, delivery_fee_part_code):
        # type: (int, OrderTakerWrapper, RemoteOrderItemCreator, OrderRepository, str) -> None
        self.pos_id = pos_id
    ...
    def finalize_remote_order(self, remote_order):
        ...
        with self.order_taker_wrapper_lock:
            order_id = int(remote_order.custom_properties["local_order_id"].value)
    ...
    def get_order_custom_property_value(self, key, order_id):
        # type: (unicode, unicode) -> Optional[str]
        custom_property_xml = eTree.XML(self.order_taker_wrapper.get_order_custom_property(self.pos_id, order_id, key))
        custom_property = custom_property_xml.find("OrderProperty")
        if custom_property is None:
            return None
        
        return custom_property.get("key")
