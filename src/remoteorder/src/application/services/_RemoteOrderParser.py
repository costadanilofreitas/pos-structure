...
class RemoteOrderParser(object):
    ...
    def _validate_json_pickup(json_pickup):
        ...
        if pickup_type and pickup_type not in ["delivery", "eat_in", "take_out", "drive_thru", "eat_out"]:
    ...
    def _build_remote_order(self, parsed_json):
        ...
        if "custom_properties" in parsed_json:
            remote_order.custom_properties = self._parse_custom_properties(parsed_json,
    ...
    def _parse_custom_properties(self, parsed_json, partner, need_logistics):
        # type: (dict) -> dict

        all_custom_properties = {}
        json_custom_properties = parsed_json.get("custom_properties")
    ...
    def _get_order_discounts(self, parsed_json, remote_order):
        ...
        for discount_type in store_discount:
            if discount_type == 'delivery_fee' and not self.use_delivery_fee:
                continue
            ...
            try:
                ...
                logger.info("Unhandled discount {}".format(discount_type))
