...
class RemoteOrderRuptureUpdater(object):
    ...
    def _build_rupture_json_data(self, product_list, snapshot_id, is_full):
        ...
        rupture_dict = OrderedDict([
                ('snapshot', str(snapshot_id)),
                ('merchantId', str(self.store_id)),
                ('full', bool(is_full)),
                ('items', self._convert_to_items(product_list)),
                ('ruptureCleanTime', self.rupture_clean_time_window.get("start"))
        ])

        data = json.dumps(rupture_dict)
        return data

    @staticmethod
    def _convert_to_items(product_list):
        active = [item["partcode"] for item in product_list if item["active"]]
        paused = [item["partcode"] for item in product_list if not item["active"]]
