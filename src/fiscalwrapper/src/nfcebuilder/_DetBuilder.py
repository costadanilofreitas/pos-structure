...
from helper import unicode_2_ascii

from pos_model import Order, TaxItem, SaleItem
...
class DetBuilder(NfceXmlPartBuilder):
    ...
    def build_xml(self, order, context):
        ...
        cst_icms = self.fiscal_parameter_controller.get_parameter(product_code, "CST_ICMS")
        if cst_icms is None:
            raise Exception("CST ICMS Não encontrado\\%s-%s" % (product_code, sale_item.product_name))
        cst_icms = int(cst_icms)

        cst_pis = self.fiscal_parameter_controller.get_parameter(product_code, "CST_PIS")
        if cst_pis is None:
            raise Exception("CST PIS Não encontrado\\%s-%s" % (product_code, sale_item.product_name))
        cst_pis = int(cst_pis)

        cst_cofins = self.fiscal_parameter_controller.get_parameter(product_code, "CST_COFINS")
        if cst_cofins is None:
            raise Exception("CST COFINS Não encontrado\\%s-%s" % (product_code, sale_item.product_name))
        cst_cofins = int(cst_cofins)
        ...
        if self.ambiente == '1':
            product_name = unicode_2_ascii(escape(sale_item.product_name).strip())
        else:
