...
def _build_order_identifier(report, print_order_identifier, order):
    if print_order_identifier:
        ...
        report += _get_delivery_partner_and_short_reference(order)
        report += "\n" + constants.SEPARATOR_DASH + "\n"
    return report
...
def _get_delivery_partner_and_short_reference(order):
    delivery_partner = order.find("CustomOrderProperties/OrderProperty[@key='PARTNER']")
    if delivery_partner is not None:
        short_reference = order.find("CustomOrderProperties/OrderProperty[@key='SHORT_REFERENCE']")
        if short_reference is not None:
            return "\n{}: {}".format(delivery_partner.get("value").upper(), short_reference.get("value").upper())
