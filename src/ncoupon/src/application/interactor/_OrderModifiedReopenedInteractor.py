...
class OrderModifiedReopenedInteractor(object):
    ...
    def remove_benefit(self, pos_id, order_id, order_picture):
        # type: (str, str, eTree) -> None

        order_benefits_id = find_order_benefits_id(self.logger, order_picture)
        
        if len(order_benefits_id) > 0:
            self.logger.info("Benefits founded to remove: {}".format(order_benefits_id))
            pos_id = replace_pos_id_by_originator_id(self.logger, pos_id, order_picture)
        
        if order_benefits_id:
            self.logger.info("Removing benefit for posId: {}; orderId: {}".format(pos_id, order_id))
            self.benefit_appliers_repositories[BenefitAppliers.N_DISCOUNT].remove_benefit(pos_id)
