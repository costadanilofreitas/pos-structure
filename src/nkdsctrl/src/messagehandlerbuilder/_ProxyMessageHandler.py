...
TK_KDS_RELOAD = TokenCreator.create_token(TokenPriority.high, MSG_GRP_KDS, "14")
...
class ProxyMessageHandler(MessageHandler):
    ...
    def handle_message(self, message_bus, message):
        ...
        elif message.token == TK_KDS_RELOAD:
            resp = self.handle_kds_reload(data, message_bus)
        else:
            resp = self.forward_message(message, message_bus)
    ...
    def handle_kds_reload(self, data, message_bus):
        self.logger.info("TK_KDS_RELOAD - Received")

        production_view = data[0]
        for kds in self.get_kds_list_with_production_view(production_view):
            self.logger.info("TK_KDS_RELOAD - Sending event for KDS: {}".format(kds))

            event_subject = "KDS" + kds
            event_type = "RELOAD_" + production_view
            refresh_end_xml = eTree.XML("<Reload name=\"{}\"/>".format(production_view))
            event = self.send_event(event_subject, event_type, refresh_end_xml)
            message_bus.publish_event(event)

        self.logger.info("TK_KDS_RELOAD - Finished")
    ...
    def forward_message(self, message, message_bus):
        resp = NakMessage('', 0)
        try:
            self.logger.info("Receiving message for KDS - data = {}".format(message))
            resp = message_bus.send_message(self.comp_name, message)
            self.logger.info("Message forwarded")
        except MessageBusException: # noqa
            self.logger.info("Error forwarding message - Component not found")
