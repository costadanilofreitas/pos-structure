...
class KdsModel(object):
    ...
    def _create_cell_format(view, view_xml):
        header_macro = view.header_text_macro
        footer_macro = view.footer_text_macro
        eTree.SubElement(view_xml, "CellFormat", header=header_macro, footer=footer_macro, body=view.body_text_macro)

    @staticmethod
    def _create_layout(view_xml, zoom):
        eTree.SubElement(view_xml, "Layout", rows=zoom.rows, lines=zoom.lines, cols=zoom.cols, fontsize=zoom.fontsize)
